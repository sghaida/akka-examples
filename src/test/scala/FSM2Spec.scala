...
class FSM2Spec extends TestKit(ActorSystem("system"))with ImplicitSender with WordSpecLike with BeforeAndAfterAll {

  override def afterAll(): Unit = TestKit.shutdownActorSystem(system)

  import com.sghaida.akka.actors.patterns.AkkaFSM._

  " vending machine" should {
    "error when not initialized" in {

      val machine = system.actorOf(Props[VendingMachineFSM])
    ...
    "report product not available" in {
      val machine = system.actorOf(Props[VendingMachineFSM])
      machine ! Initialized(Map("chips"-> 10), Map("chips"-> 10))
    ...
    "handle recipient of partial money" in {
      val machine = system.actorOf(Props[VendingMachineFSM])
      machine ! Initialized(Map("xxx"-> 10), Map("xxx"-> 10))
      machine ! RequestProduct("xxx")
      expectMsg(Instruction("please insert price: 10"))
    ...
    "deliver the product if insert all the money" in {
      ...
      val machine = system.actorOf(Props[VendingMachineFSM])
      machine ! Initialized(Map("aaa"-> 10), Map("aaa"-> 10))
    ...
    "deliver the product and give back change" in {
      ...
      val machine = system.actorOf(Props[VendingMachineFSM])
      machine ! Initialized(Map("bbb"-> 10), Map("bbb"-> 10))
